...
// SHOW LAST SE X LAST LOGGED IN USERS
...
$query  = "SELECT *, FROM_UNIXTIME((user_lastlogindate), '%d/%m/%y %H:%i') as login,
FROM_UNIXTIME((user_lastlogindate), '%d/%m/%y') as login1, FROM_UNIXTIME((user_lastlogindate), '%H:%i') as login2
FROM se_users WHERE user_search='1' AND user_invisible='0' ORDER BY user_lastlogindate DESC limit ".$numOfUser."";
...
if($date_format=="4") {
$miovalore= giggitime($row['user_lastlogindate'], $num_times=1).' ago';
} else if ($date_format=="1") {
$miovalore=$row['login'];
} else if ($date_format=="2") {
$miovalore=$row['login1'];
} else if ($date_format=="3") {
$miovalore=$row['login2'];
}
...
$mydir=$wpdir."/wp-content/plugins/wp-se_lastlogged";
